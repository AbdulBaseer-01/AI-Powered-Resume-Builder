...
function SkillPreview({ resumeInfo }) {
  return (
    <div className="my-4 px-2">
      <h2 className="section-title" style={{ color: resumeInfo?.themeColor || '#06c8eb' }}>
        SKILLS
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
        {resumeInfo?.skills?.map((skill, index) => {
          const color = resumeInfo?.themeColor || '#06c8eb';
          ...
          return (
            <div key={index} className="flex flex-col gap-1">
              <div className="flex justify-between font-medium">
                <span className="truncate">{skill.name}</span>
                <span className="text-gray-700">{getRatingLabel(rating)}</span>
              </div>
              <div
                className="h-2 bg-gray-200 rounded-sm overflow-hidden border"
